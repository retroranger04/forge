"""Axis C coverage evaluation: the frozen run_01 model against seven pre-stress levels.
...
# Session 6.3 spec values, to 6 dp; the exact thresholds come from SESSION_SIX.
...
def main():
    ...
    for p_percent in P_VALUES:
        ...
        if p_percent == CONTROL_P:
            ...
            print(f"control check: P=0.0 cov@90={row['coverage_at_90']:.4f} vs Session Six "
                  f"{id_cov90_reference:.4f}, |delta|={off:.4f} "
                  f"(tolerance {CONTROL_TOLERANCE})", flush=True)
            if off > CONTROL_TOLERANCE:
                results["halted"] = "P=0.0 control coverage off Session Six ID coverage"
                flush_results()
                raise SystemExit(
