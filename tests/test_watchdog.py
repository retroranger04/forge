...
def write_aged(log, worker, event, age_sec, phase="train", run_id=RUN, **kv):
    ...
    pairs = " ".join(f"{k}={v}" for k, v in {"run_id": run_id, **kv}.items())
    log.parent.mkdir(parents=True, exist_ok=True)
    with log.open("a", encoding="utf-8") as f:
        f.write(f"{ts} [{worker}] {phase}/{event} {pairs}\n")


def test_all_complete_workers_report_complete(tmp_path):
    log = tmp_path / "run.log"
    for w in ("w1", "w2", "w3"):
        emit(log, w, RUN, "train", "progress", i=100)
        emit(log, w, RUN, "train", "complete", saved=2000)

    wd = Watchdog(log, {"w1", "w2", "w3"}, RUN, stale_sec=5, poll_interval_sec=60)
...
def test_worker_that_stops_emitting_is_stalled(tmp_path):
    log = tmp_path / "run.log"
    emit(log, "w1", RUN, "train", "complete", saved=10)
    write_aged(log, "w2", "progress", age_sec=30, i=400)  # went quiet 30s ago

    wd = Watchdog(log, {"w1", "w2"}, RUN, stale_sec=5, poll_interval_sec=60)
...
def test_watchdog_exits_when_all_workers_terminal(tmp_path):
    ...
    t = threading.Thread(
        target=lambda: result.update(
            Watchdog(log, {"w1", "w2"}, RUN, stale_sec=5, poll_interval_sec=60)
...
def test_budget_exceeded_for_still_active_worker(tmp_path):
    ...
    wd = Watchdog(log, {"w1"}, RUN, stale_sec=60, budget_sec=100, poll_interval_sec=60)
...
def test_worker_that_never_appears_is_missing(tmp_path):
    ...
    wd = Watchdog(log, {"w1", "ghost"}, RUN, stale_sec=0, poll_interval_sec=60)
...
def test_on_stall_fires_even_if_warning_logging_fails(tmp_path):
    ...
    calls = []
    try:
        wd = Watchdog(log, {"w1"}, RUN, stale_sec=5, poll_interval_sec=60,
...
def test_watchdog_logs_a_warning_event(tmp_path):
    ...
    assert warnings[0]["run_id"] == RUN
    assert warnings[0]["fields"]["worker"] == "w1"


def test_watchdog_ignores_entries_from_other_run_id(tmp_path):
    log = tmp_path / "run.log"
    # a previous run left this file complete and fresh
    emit(log, "w1", "old_run", "train", "complete", saved=10)
    emit(log, "w1", "old_run", "train", "progress", i=999)
    # ...and an untagged line from before run_id existed at all
    with log.open("a", encoding="utf-8") as f:
        ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        f.write(f"{ts} [w1] train/progress i=999\n")

    # this run's w1 never emitted, so neither the stale run's completion nor
    # its fresh progress may make it look alive
    wd = Watchdog(log, {"w1"}, RUN, stale_sec=0, poll_interval_sec=60)
    assert wd.poll() == {"w1": "missing"}

    # once this run does emit, it is tracked on its own lines only
    emit(log, "w1", RUN, "train", "progress", i=1)
    wd2 = Watchdog(log, {"w1"}, RUN, stale_sec=60, poll_interval_sec=60)
    assert wd2.poll() == {"w1": "running"}
