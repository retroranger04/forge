...
def test_emitted_line_is_parseable(tmp_path):
    ...
    emit(log, "train_shard_2", "test_run", "train", "progress", i=1400, N=2000, elapsed_sec=612)

    entries = parse_log(log)
    assert len(entries) == 1
    e = entries[0]
    assert e["worker_id"] == "train_shard_2"
    assert e["run_id"] == "test_run"
    ...
    assert raw.endswith(
        "[train_shard_2] train/progress run_id=test_run i=1400 N=2000 elapsed_sec=612"
    )
...
def test_concurrent_writes_are_not_lost(tmp_path):
    ...
    def worker(n):
        ...
        emit(log, f"w{n}", "test_run", "sample_gen", "heartbeat", i=n)
...
def test_malformed_lines_are_skipped(tmp_path):
    log = tmp_path / "run.log"
    emit(log, "w1", "test_run", "train", "progress", i=1)
    with log.open("a", encoding="utf-8") as f:
        ...
        f.write("\n")
    emit(log, "w4", "test_run", "train", "complete", i=5)
...
def test_line_breaking_values_are_rejected(tmp_path):
    ...
    for bad in ("solver failed\nretrying", "carriage\rreturn"):
        with pytest.raises(ValueError):
            emit(log, "w1", "test_run", "train", "error", msg=bad)
...
def test_field_names_that_break_parsing_are_rejected(tmp_path):
    ...
    for bad in ({"bad name": 1}, {"has=equals": 1}, {"1leading_digit": 1}):
        with pytest.raises(ValueError):
            emit(log, "w1", "test_run", "train", "progress", **bad)


def test_values_with_spaces_survive_round_trip(tmp_path):
    log = tmp_path / "run.log"
    emit(log, "w1", "test_run", "train", "error", msg="solver did not converge", code=3)

    entries = parse_log(log)
    assert entries[0]["fields"] == {"msg": "solver did not converge", "code": "3"}


def test_logging_run_id_field_roundtrips(tmp_path):
    log = tmp_path / "run.log"
    emit(log, "trainer", "run_01", "train", "progress", step=100)
    emit(log, "trainer", "run_02", "train", "progress", step=100)
    with log.open("a", encoding="utf-8") as f:  # legacy line, no run_id pair
        f.write("2026-08-09T14:32:17Z [trainer] train/progress step=100\n")

    entries = parse_log(log)
    assert [e["run_id"] for e in entries] == ["run_01", "run_02", None]
    # run_id is lifted out of the k=v pairs, not left to shadow a real field
    assert all(e["fields"] == {"step": "100"} for e in entries)
